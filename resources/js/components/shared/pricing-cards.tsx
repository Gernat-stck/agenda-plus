...
const Feature = memo(({ feature }: { feature: Feature }) => {
  ...
  return (
    <li className={cn("flex items-center text-sm sm:text-base", !feature.included && "text-gray-400")}>
      {icon}
      <span className="text-sm sm:text-base text-gray-700 dark:text-gray-300">{feature.text}</span>
    </li>
  )
})
  ...
  ({
    ...
    return (
      <Card
        ref={cardRef}
        className={cn(
          "absolute cursor-pointer  transition-all duration-500 ease-in-out w-[280px] xs:w-[320px] sm:w-[350px] md:w-[380px] min-h-[450px]",
          neonClass,
        ...
        onClick={onClick}
      >
        <div className="relative overflow-hidden flex flex-col h-full  rounded-lg">
          {plan.badge && (
            <div className="absolute top-0 right-0 z-10">
              <UIBadge
                className={cn(
                  "px-4 py-1 text-sm font-medium rounded-bl-md rounded-tr-md rounded-br-none rounded-tl-none",
                  isPopular ? "bg-black text-white dark:bg-white dark:text-black" : "bg-gray-800 text-white dark:bg-white dark:text-black",
                )}
              >
                {plan.badge.text}
              </UIBadge>
          ...
          <CardHeader className="pb-2 sm:pb-4 pt-4 sm:pt-6">
            <CardTitle className="text-xl sm:text-2xl font-bold">{plan.title}</CardTitle>
            <CardDescription className="text-gray-500 mt-1 text-sm sm:text-base">{plan.description}</CardDescription>
            <div className="mt-3 sm:mt-5 flex items-baseline">
              <span className="text-3xl sm:text-4xl font-bold">{plan.price}</span>
              <span className="text-gray-500 ml-1 text-sm sm:text-base font-normal">{plan.period}</span>
            </div>
          </CardHeader>
          <CardContent className="pb-4 sm:pb-6 flex-1">
...
export function PricingCards({ plans, defaultActiveCard = 1, onCardSelect }: PricingCardsProps) {
  ...
  return (
    <div className="w-full ">
      <div
        ref={containerRef}
        className="relative mx-auto w-full max-w-[1200px] min-h-[550px] sm:min-h-[600px] md:min-h-[650px] flex items-center justify-center overflow-hidden"
