        ...
        <Popover>
            <PopoverTrigger asChild>
                <Button
                    type="button"
                    variant="outline"
                    className="flex gap-1 rounded-s-lg rounded-e-none border-r-0 bg-transparent px-3 focus:z-10"
                    disabled={disabled}
                >
...
const FlagComponent = ({ country, countryName }: RPNInput.FlagProps) => {
    ...
    return <span className="flex h-4 w-6 overflow-hidden rounded-sm bg-transparent [&_svg]:size-full">{Flag && <Flag title={countryName} />}</span>;
};
